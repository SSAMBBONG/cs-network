...
- 네트워크 계층에서의 PDU는 데이터와 헤더를 합친것 (패킷)
...
1. 특정 포트와 해당 포트에 연결된 호스트의 MAC 주소와의 관계를 기억하는데 이를 `MAC 주소 학습`이라 함
2. 이를 통해 원하는 호스트에만 프레임을 전달할 수 있음
...
- 필터링
    ...
    - 각 호스트들은 자신과 관련이 없는 프레임을 전송받았으면 이를 폐기하고 MAC 주소 테이블에 저장함
    - 이렇게 저장된 MAC 주소 테이블을 이용해 필터링을 함
- 포워딩
