...
패킷에 할당된 번호다. IPv4 패킷이 여러 조각으로 쪼개져 전송되었을 때 조합을 해야하고 이떄 식별자를 사용하여 패킷을 구분한다.
...
>
> 호스트 A와 연결된 라우터 A의 MAC 주소를 얻은 후 패킷을 전송한다.
...
클래스는 A, B, C, D, E가 있으며 D와 E는 멀티캐스를 위한 클래스, 예약된 클래스다.
